...
The current native client uses only standard/exempt operating-system encryption through HTTPS and Expo SecureStore; it implements no proprietary cryptography. Expo therefore emits `ITSAppUsesNonExemptEncryption=false` for iOS. Reassess that declaration before adding any native cryptographic library or custom encryption behavior; export classification remains the application owner's responsibility.
