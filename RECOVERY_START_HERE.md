...
M2.2a is accepted in `main` at `a468fb6d3d850757fe1af82c18f244e95ad77de8`, with exact-main Foundation CI green. M2.2b is in a corrected second-review state on `m2-2b-real-scope-authority-pilot`: current NCBE Content Scope/Blueprint provenance and a one-leaf Rule 4 pilot are deterministic and body-free, but all eight V2 obligations remain `REVIEW_REQUIRED` and certification is blocked. Read `docs/project/M2_2B_HUMAN_REVIEW_PACKET.md`; V1 correction history is preserved separately. Do not fabricate approval, certify, merge, or begin the next slice. Apple enrollment/signing, physical-device authentication/SecureStore, live OS association routing, store ownership/approval, and native production purchase/restore remain mandatory at the applicable Native GA and are not marked verified.
