...
Review date: 2026-09-04. Re-review on each Expo SDK or React Native patch upgrade and before a native release candidate. This ledger records `npm audit` reachability; it is not a claim that scanner findings are false.
...
The Expo 57.0.18/React Native 0.86.3 compatibility correction updated Metro from 0.84.4 to 0.84.5 and removed `image-size` from the installed production graph. The two high-severity `image-size` advisories and their five high aggregate nodes therefore remain closed. Expo 57.0.20 leaves React Navigation package versions unchanged; the 17-moderate aggregate remains current npm advisory metadata across the same two families. No compatible non-breaking fix for either remaining family is available.
