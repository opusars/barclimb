...
## Current state
M2.2b is accepted in `main` at `2b042d158bb68307a0af223acec1542b62702d20`; exact-main Foundation CI run `32522925767` passed. M2.2c is active on `m2-2c-civpro-subject-foundation`. The branch identifies all six official Civil Procedure leaves and proposes a versioned subject manifest, 18 curriculum-layer coverage requirements, 75 typed structural slots, 16 authority-family plans, and a noncircular explicit certification gate. The existing Rule 4 V2 snapshot is linked unchanged as `PARTIAL_LEAF_COVERAGE`. The subject plan remains `REVIEW_PENDING`; zero new substantive Rule Obligations exist, zero subject slots are certified, `subject_complete` is false, and `national_complete` is false. A qualified human must review `M2_2C_HUMAN_REVIEW_PACKET.md` before this plan may become approved production planning truth. Accepted M1 platform evidence and deferred Native GA blockers remain unchanged.
...
- Authoritative accepted main is `2b042d158bb68307a0af223acec1542b62702d20`; exact-main Foundation CI run `32522925767` passed before M2.2c began.
- Active branch is `m2-2c-civpro-subject-foundation`. Finish validation, commit/push, and exact-commit CI, but do not merge automatically. The human-review packet remains pending and no substantive subject-wide candidate compilation may begin.
...
- Leo Rayos (JD; California bar exam passer; reviewer for BarClimb curriculum quality control) reviewed the hash-identified December 1, 2025 FRCP authority and approved all eight V2 obligations for the bounded one-leaf pilot. The body-free manifest records those decisions through `record_obligation_review` without fabricating an account.
- Fresh reconciliation has zero duplicate, omission, excess, conflict, ambiguity, provenance, jurisdiction, warning, or blocking issues; one of one target leaves is sufficient; the compiler decisions remain `REVIEW_REQUIRED` and are satisfied by the eight immutable approvals; `national_complete` is false.
...
## M2.2c Civil Procedure subject foundation
- `BARCLIMB_CIVPRO_CURRICULUM_MANIFEST@2026_V1` is bound to real scope `NCBE_NEXTGEN_SCOPE_2026_07_2027_02`; manifest SHA-256 is `8fd6506bfc4cfda72e1ee6aaad6d62f8ab233fb1c48ff45c308e01b12b60ebd8`.
- Exactly six official Civil Procedure leaves are preserved by identity, label, hierarchy, locator, and treatment. Curriculum decomposition exists only in immutable `ScopeCoverageRequirement` records and never changes NCBE truth.
- Eighteen requirements expand into 75 typed slots. A future certified obligation must match its slot kind, official leaf, compiler/reconciliation truth, and immutable snapshot. Counts or percentages cannot certify a leaf or subject.
- Sixteen authority plans cover actually applicable constitutional text, Title 28/Rules Enabling Act statutes, FRCP, FRAP, Supreme Court case authority, and optional secondary reconciliation. Only the accepted FRCP family is acquired; 15 remain planned. Five case plans require exact identity/citation, court/date, reliable URI, proposition locator, authority status, and later-treatment review.
- The Erie/Hanna requirement proves one complex leaf can demand constitutional, statutory, rules-enabling, federal-rule, and controlling case propositions plus explicit distinctions. Procedural requirements separately demand prerequisites, steps, branching, timing/limitations, and remedies.
- The federal/state rule is explicit: nationally testable federal incorporation/comparison doctrine remains core; standalone state-specific procedure remains prohibited.
- Existing Rule 4 compile `0148dea...` and snapshot `8ffc025a...` link unchanged as `PILOT_ONLY` partial evidence. They satisfy no subject slot automatically and do not complete the service leaf.
- `report_subject_coverage` reports six uncovered leaves, 75 pending slots, 15 authority gaps, pending human review, `subject_complete: false`, and `national_complete: false`. M2.2c implements no subject-certification snapshot or hidden certification command.
- `M2_2C_HUMAN_REVIEW_PACKET.md` is the required review gate. Its current SHA-256 is `153746608c27abad008dbfa5cd858113c746696ed05fc410a1b6242e558b1f6c`. No approval is recorded or inferred.

## Tests actually run
- M2.2c local acceptance: clean hash-pinned Python dependencies installed successfully; Ruff, Django system/migration checks, 107 SQLite tests with 10 PostgreSQL-only skips, all 117 PostgreSQL 14/Redis tests, and live Celery smoke passed. Exact Node 24.19.0/npm 11.17.0 clean install, all nine typechecks, 14 Web tests, 20 native tests, 8 schema tests, Web build, seven-package portability, Expo compatibility, Doctor 20/20, and iOS/Android exports at 863/858 modules passed. Audit remains the accepted 15-node `image-size`/`uuid` graph. Final continuity/diff/scans and exact-commit CI remain.
...
## Exact next task
Complete full M2.2c local/PostgreSQL/multi-client validation, final continuity/diff review, commit and push `m2-2c-civpro-subject-foundation`, and require exact-commit Foundation CI green. Then obtain qualified human review of `M2_2C_HUMAN_REVIEW_PACKET.md`. Do not approve the plan without that supplied review, do not merge automatically, and do not begin substantive Civil Procedure candidate compilation.
