...
At Milestone 1/M1.4, deep-link route architecture/configuration, native authentication protocol boundaries, automated secure-credential state coverage, real navigation, and one available signed native internal-build path are sufficient foundation proof when the remaining platform accounts or devices are externally unavailable. This does not prove actual-device authentication/SecureStore lifecycle or live OS link routing. Published AASA/`assetlinks.json` associations, exact-object routing on real devices, platform-final signing, and the applicable store/account evidence remain mandatory before the relevant iOS or Android Native GA and stay explicitly `NOT_VERIFIED` or `BLOCKED_EXTERNAL` until observed.
